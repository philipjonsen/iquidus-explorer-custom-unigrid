...
(function ($) {
  ...
  let _colorpicker_index = 0
  const _container_popup =
    '<div class="ui-colorpicker ui-colorpicker-dialog ui-dialog ui-widget ui-widget-content ui-corner-all" style="display: none;"></div>'
  const _container_inline =
    '<div class="ui-colorpicker ui-colorpicker-inline ui-dialog ui-widget ui-widget-content ui-corner-all"></div>'
  const _parts_lists = {
    full: [
      'header',
      'map',
  ...
  const _intToHex = function (dec) {
    let result = Math.round(dec).toString(16)
    if (result.length === 1) {
      result = '0' + result
    }
  ...
  var _formats = {
    ...
    '#HEX3': function (color) {
      const hex3 = _formats.HEX3(color)
  ...
  var _closestName = function (color) {
    const rgb = color.getRGB()
    let distance = null
    let name = false
    let d
  ...
  const _parseHex = function (color) {
    let c, m
  ...
  const _parseColor = function (color) {
    const name = $.trim(color).toLowerCase()
    let m
    ...
    // hsla(r,g,b,a)
    m =
      /^hsla?\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*(?:,\s*(\d+(?:\.\d+)?)\s*)?\)$/.exec(
        color
      )
    if (m) {
      return new Color()
        .setHSL(m[1] / 255, m[2] / 255, m[3] / 255)
        .setAlpha(parseFloat(m[4]))
    }
    ...
    if (m) {
      return new Color()
        .setHSL(m[1] / 100, m[2] / 100, m[3] / 100)
        .setAlpha(m[4] / 100)
    }
  ...
  const _layoutTable = function (layout, callback) {
    let bitmap,
      x,
      y,
      width,
    ...
    for (y = 0; y < height; ++y) {
      ...
      for (x = 0; x < width; x) {
        if (
          ...
          while (
            x < width &&
            bitmap[x][y] === undefined &&
            (cell === undefined ||
              y < cell.pos[1] ||
              (y == cell.pos[1] && x < cell.pos[0]))
          ) {
            if (columns[x] === true) {
              colspan += 1
            }
  ...
  const _parts = {
    header: function (inst) {
      ...
      const _html = function () {
        const title = inst.options.title || inst._getRegional('title')
        let html = '<span class="ui-dialog-title">' + title + '</span>'
      ...
      this.init = function () {
        ...
        const close = $('.ui-dialog-titlebar-close', e)
    ...
    map: function (inst) {
      ...
      _mousedown = function (event) {
        ...
        const div = $('.ui-colorpicker-map-layer-pointer', e)
      ...
      _html = function () {
        const html =
          '<div class="ui-colorpicker-map ui-colorpicker-border">' +
          '<span class="ui-colorpicker-map-layer-1">&nbsp;</span>' +
          '<span class="ui-colorpicker-map-layer-2">&nbsp;</span>' +
          (inst.options.alpha
            ? '<span class="ui-colorpicker-map-layer-alpha">&nbsp;</span>'
            : '') +
      ...
      this.update = function () {
        switch (inst.mode) {
          case 'h':
            $('.ui-colorpicker-map-layer-1', e)
              .css({ 'background-position': '0 0', opacity: '' })
              .show()
          ...
          case 'a':
            $('.ui-colorpicker-map-layer-1', e)
              .css({ 'background-position': '0 -260px', opacity: '' })
              .show()
            $('.ui-colorpicker-map-layer-2', e)
              .css({ 'background-position': '0 -520px', opacity: '' })
              .show()
            break
          ...
          case 'v':
            ...
            $('.ui-colorpicker-map-layer-1', e)
              .css({ 'background-position': '0 -780px', opacity: '' })
              .show()
            $('.ui-colorpicker-map-layer-2', e).hide()
            break
          ...
          case 'r':
            $('.ui-colorpicker-map-layer-1', e)
              .css({ 'background-position': '0 -1040px', opacity: '' })
              .show()
            $('.ui-colorpicker-map-layer-2', e)
              .css({ 'background-position': '0 -1300px', opacity: '' })
              .show()
            break

          case 'g':
            $('.ui-colorpicker-map-layer-1', e)
              .css({ 'background-position': '0 -1560px', opacity: '' })
              .show()
            $('.ui-colorpicker-map-layer-2', e)
              .css({ 'background-position': '0 -1820px', opacity: '' })
              .show()
            break

          case 'b':
            $('.ui-colorpicker-map-layer-1', e)
              .css({ 'background-position': '0 -2080px', opacity: '' })
              .show()
            $('.ui-colorpicker-map-layer-2', e)
              .css({ 'background-position': '0 -2340px', opacity: '' })
              .show()
            break
        }
        that.repaint()
      ...
      this.repaint = function () {
        ...
        if (inst.options.alpha) {
          $('.ui-colorpicker-map-layer-alpha', e).css(
            'opacity',
            1 - inst.color.getAlpha()
          )
        }
      ...
      this.init = function () {
        e = $(_html()).appendTo(
          $('.ui-colorpicker-map-container', inst.dialog)
        )
    ...
    bar: function (inst) {
      ...
      _mousedown = function (event) {
        ...
        const div = $('.ui-colorpicker-bar-layer-pointer', e)
        const offset = div.offset()
        const width = div.width()
        const height = div.height()
        const x = event.pageX - offset.left
      ...
      _html = function () {
        let html =
          '<div class="ui-colorpicker-bar ui-colorpicker-border">' +
        ...
        if (inst.options.alpha) {
          html +=
            '<span class="ui-colorpicker-bar-layer-alpha">&nbsp;</span>' +
            '<span class="ui-colorpicker-bar-layer-alphabar">&nbsp;</span>'
        }

        html +=
          '<span class="ui-colorpicker-bar-layer-pointer"><span class="ui-colorpicker-bar-pointer"></span></span></div>'
        ...
        switch (inst.mode) {
          ...
          case 'r':
            ...
            $('.ui-colorpicker-bar-layer-2', e)
              .css({ 'background-position': '0 -1300px', opacity: '' })
              .show()
            $('.ui-colorpicker-bar-layer-3', e)
              .css({ 'background-position': '0 -780px', opacity: '' })
              .show()
            $('.ui-colorpicker-bar-layer-4', e)
              .css({ 'background-position': '0 -1040px', opacity: '' })
              .show()
            break
          ...
          case 'g':
            $('.ui-colorpicker-bar-layer-1', e)
              .css({ 'background-position': '0 -2600px', opacity: '' })
              .show()
            $('.ui-colorpicker-bar-layer-2', e)
              .css({ 'background-position': '0 -2340px', opacity: '' })
              .show()
          ...
          case 'b':
            $('.ui-colorpicker-bar-layer-1', e)
              .css({ 'background-position': '0 -3640px', opacity: '' })
              .show()
            $('.ui-colorpicker-bar-layer-2', e)
              .css({ 'background-position': '0 -3380px', opacity: '' })
              .show()
            $('.ui-colorpicker-bar-layer-3', e)
              .css({ 'background-position': '0 -2860px', opacity: '' })
              .show()
            $('.ui-colorpicker-bar-layer-4', e)
              .css({ 'background-position': '0 -3120px', opacity: '' })
              .show()
            break
      ...
      this.repaint = function () {
        ...
        switch (inst.mode) {
          ...
          case 'r':
            ...
            $('.ui-colorpicker-bar-layer-2', e).css(
              'opacity',
              Math.max(0, inst.color.getRGB().b - inst.color.getRGB().g)
            )
            $('.ui-colorpicker-bar-layer-3', e).css(
              'opacity',
              Math.max(0, inst.color.getRGB().g - inst.color.getRGB().b)
            )
            $('.ui-colorpicker-bar-layer-4', e).css(
              'opacity',
              Math.min(inst.color.getRGB().b, inst.color.getRGB().g)
            )
            break
          ...
          case 'g':
            ...
            $('.ui-colorpicker-bar-layer-2', e).css(
              'opacity',
              Math.max(0, inst.color.getRGB().b - inst.color.getRGB().r)
            )
            $('.ui-colorpicker-bar-layer-3', e).css(
              'opacity',
              Math.max(0, inst.color.getRGB().r - inst.color.getRGB().b)
            )
            $('.ui-colorpicker-bar-layer-4', e).css(
              'opacity',
              Math.min(inst.color.getRGB().r, inst.color.getRGB().b)
            )
            break
          ...
          case 'b':
            ...
            $('.ui-colorpicker-bar-layer-2', e).css(
              'opacity',
              Math.max(0, inst.color.getRGB().r - inst.color.getRGB().g)
            )
            $('.ui-colorpicker-bar-layer-3', e).css(
              'opacity',
              Math.max(0, inst.color.getRGB().g - inst.color.getRGB().r)
            )
            $('.ui-colorpicker-bar-layer-4', e).css(
              'opacity',
              Math.min(inst.color.getRGB().r, inst.color.getRGB().g)
            )
            break
        ...
        if (inst.mode !== 'a') {
          $('.ui-colorpicker-bar-layer-alpha', e).css(
            'opacity',
            1 - inst.color.getAlpha()
          )
        }
      ...
      this.init = function () {
        e = $(_html()).appendTo(
          $('.ui-colorpicker-bar-container', inst.dialog)
        )
    ...
    preview: function (inst) {
      ...
      this.init = function () {
        e = $(_html()).appendTo(
          $('.ui-colorpicker-preview-container', inst.dialog)
        )
      ...
      this.repaint = function () {
        $('.ui-colorpicker-preview-initial', e)
          .css('background-color', inst.currentColor.toCSS())
          .attr('title', inst.currentColor.toHex())
        $('.ui-colorpicker-preview-initial-alpha', e).css(
          'opacity',
          1 - inst.currentColor.getAlpha()
        )
        $('.ui-colorpicker-preview-current', e)
          .css('background-color', inst.color.toCSS())
          .attr('title', inst.color.toHex())
        $('.ui-colorpicker-preview-current-alpha', e).css(
          'opacity',
          1 - inst.color.getAlpha()
        )
      }
    ...
    hsv: function (inst) {
      ...
      _html = function () {
        ...
        if (inst.options.hsv) {
          html +=
            '<div class="ui-colorpicker-hsv-h"><input class="ui-colorpicker-mode" type="radio" value="h"/><label>' +
      ...
      this.init = function () {
        e = $(_html()).appendTo(
          $('.ui-colorpicker-hsv-container', inst.dialog)
        )
      ...
      this.repaint = function () {
        ...
        $.each(hsv, function (index, value) {
          const input = $(
            '.ui-colorpicker-hsv-' + index + ' .ui-colorpicker-number',
            e
          )
    ...
    rgb: function (inst) {
      ...
      _html = function () {
        ...
        if (inst.options.rgb) {
          html +=
            '<div class="ui-colorpicker-rgb-r"><input class="ui-colorpicker-mode" type="radio" value="r"/><label>' +
            inst._getRegional('rgbR') +
            '</label><input class="ui-colorpicker-number" type="number" min="0" max="255"/></div>' +
            '<div class="ui-colorpicker-rgb-g"><input class="ui-colorpicker-mode" type="radio" value="g"/><label>' +
            inst._getRegional('rgbG') +
            '</label><input class="ui-colorpicker-number" type="number" min="0" max="255"/></div>' +
            '<div class="ui-colorpicker-rgb-b"><input class="ui-colorpicker-mode" type="radio" value="b"/><label>' +
      ...
      this.init = function () {
        e = $(_html()).appendTo(
          $('.ui-colorpicker-rgb-container', inst.dialog)
        )
      ...
      this.repaint = function () {
        $.each(inst.color.getRGB(), function (index, value) {
          const input = $(
            '.ui-colorpicker-rgb-' + index + ' .ui-colorpicker-number',
            e
          )
    ...
    lab: function (inst) {
      ...
      const html = function () {
        ...
        if (inst.options.hsv) {
          html +=
            '<div class="ui-colorpicker-lab-l"><label>' +
            inst._getRegional('labL') +
            '</label><input class="ui-colorpicker-number" type="number" min="0" max="100"/></div>' +
            '<div class="ui-colorpicker-lab-a"><label>' +
            inst._getRegional('labA') +
            '</label><input class="ui-colorpicker-number" type="number" min="-128" max="127"/></div>' +
            '<div class="ui-colorpicker-lab-b"><label>' +
      ...
      this.init = function () {
        ...
        part = $(html()).appendTo(
          $('.ui-colorpicker-lab-container', inst.dialog)
        )

        $('.ui-colorpicker-number', part).on('change keyup', function (event) {
          inst.color.setLAB(
            parseInt(
              $('.ui-colorpicker-lab-l .ui-colorpicker-number', part).val(),
              10
            ) / 100,
            (parseInt(
              $('.ui-colorpicker-lab-a .ui-colorpicker-number', part).val(),
              10
            ) +
              128) /
              255,
            (parseInt(
              $('.ui-colorpicker-lab-b .ui-colorpicker-number', part).val(),
              10
            ) +
              128) /
              255
      ...
      this.repaint = function () {
        ...
        $.each(lab, function (index, value) {
          const input = $(
            '.ui-colorpicker-lab-' + index + ' .ui-colorpicker-number',
            part
          )
          value = Math.round(value)
          if (input.val() !== value) {
    ...
    cmyk: function (inst) {
      ...
      const html = function () {
        ...
        if (inst.options.hsv) {
          html +=
            '<div class="ui-colorpicker-cmyk-c"><label>' +
            inst._getRegional('cmykC') +
            '</label><input class="ui-colorpicker-number" type="number" min="0" max="100"/><span class="ui-colorpicker-unit">%</span></div>' +
            '<div class="ui-colorpicker-cmyk-m"><label>' +
            inst._getRegional('cmykM') +
            '</label><input class="ui-colorpicker-number" type="number" min="0" max="100"/><span class="ui-colorpicker-unit">%</span></div>' +
            '<div class="ui-colorpicker-cmyk-y"><label>' +
            inst._getRegional('cmykY') +
            '</label><input class="ui-colorpicker-number" type="number" min="0" max="100"/><span class="ui-colorpicker-unit">%</span></div>' +
            '<div class="ui-colorpicker-cmyk-k"><label>' +
      ...
      this.init = function () {
        part = $(html()).appendTo(
          $('.ui-colorpicker-cmyk-container', inst.dialog)
        )

        $('.ui-colorpicker-number', part).on('change keyup', function (event) {
          inst.color.setCMYK(
            parseInt(
              $('.ui-colorpicker-cmyk-c .ui-colorpicker-number', part).val(),
              10
            ) / 100,
            parseInt(
              $('.ui-colorpicker-cmyk-m .ui-colorpicker-number', part).val(),
              10
            ) / 100,
            parseInt(
              $('.ui-colorpicker-cmyk-y .ui-colorpicker-number', part).val(),
              10
            ) / 100,
            parseInt(
              $('.ui-colorpicker-cmyk-k .ui-colorpicker-number', part).val(),
              10
            ) / 100
          )
          inst._change()
      ...
      this.repaint = function () {
        $.each(inst.color.getCMYK(), function (index, value) {
          const input = $(
            '.ui-colorpicker-cmyk-' + index + ' .ui-colorpicker-number',
            part
          )
    ...
    alpha: function (inst) {
      ...
      _html = function () {
        ...
        if (inst.options.alpha) {
          html +=
            '<div class="ui-colorpicker-a"><input class="ui-colorpicker-mode" name="mode" type="radio" value="a"/><label>' +
      ...
      this.init = function () {
        e = $(_html()).appendTo(
          $('.ui-colorpicker-alpha-container', inst.dialog)
        )
      ...
      this.repaint = function () {
        const input = $('.ui-colorpicker-a .ui-colorpicker-number', e)
        const value = Math.round(inst.color.getAlpha() * 100)
    ...
    hex: function (inst) {
      ...
      _html = function () {
        ...
        return (
          '<div class="ui-colorpicker-hex"><label>#</label>' + html + '</div>'
        )
      }

      this.init = function () {
        e = $(_html()).appendTo(
          $('.ui-colorpicker-hex-container', inst.dialog)
        )

        // repeat here makes the invalid input disappear faster
        $('.ui-colorpicker-hex-input', e).bind(
        ...
        $('.ui-colorpicker-hex-alpha', e).bind(
          'change keydown keyup',
          function () {
            if (/[^a-fA-F0-9]/.test($(this).val())) {
              $(this).val(
        ...
        $('.ui-colorpicker-hex-alpha', e).bind('change keyup', function () {
          inst.color.setAlpha(
            parseInt($('.ui-colorpicker-hex-alpha', e).val(), 16) / 255
          )
          inst._change()
        })
      }
      ...
      this.repaint = function () {
        ...
        if (!$('.ui-colorpicker-hex-alpha', e).is(':focus')) {
          $('.ui-colorpicker-hex-alpha', e).val(
            _intToHex(inst.color.getAlpha() * 255)
          )
        }
      }
    },
    ...
    swatches: function (inst) {
      ...
      const html = function () {
        ...
        $.each(inst.options.swatches, function (name, color) {
          ...
          const css = c.toCSS()
          html +=
            '<div class="ui-colorpicker-swatch" style="background-color: ' +
            css +
            '" title="' +
            name +
            '"></div>'
        ...
        return (
          '<div class="ui-colorpicker-swatches ui-colorpicker-border">' +
          html +
          '</div>'
        )
      }
      ...
      this.init = function () {
        part = $(html()).appendTo(
          $('.ui-colorpicker-swatches-container', inst.dialog)
        )
    ...
    footer: function (inst) {
      const that = this
      let part = null
      const id_transparent =
        'ui-colorpicker-special-transparent-' + _colorpicker_index
      const id_none = 'ui-colorpicker-special-none-' + _colorpicker_index
      const html = function () {
        let html = ''

        if (
          inst.options.alpha ||
          (!inst.inline && inst.options.showNoneButton)
        ) {
          html += '<div class="ui-colorpicker-buttonset">'

          if (inst.options.alpha) {
            html +=
              '<input type="radio" name="ui-colorpicker-special" id="' +
              id_transparent +
              '" class="ui-colorpicker-special-transparent"/><label for="' +
              id_transparent +
              '">' +
          ...
          if (!inst.inline && inst.options.showNoneButton) {
            html +=
              '<input type="radio" name="ui-colorpicker-special" id="' +
              id_none +
              '" class="ui-colorpicker-special-none"><label for="' +
              id_none +
              '">' +
              inst._getRegional('none') +
              '</label>'
          }
          html += '</div>'
        }
        ...
        if (!inst.inline) {
          ...
          if (inst.options.showCancelButton) {
            html +=
              '<button class="ui-colorpicker-cancel">' +
              inst._getRegional('cancel') +
              '</button>'
          }
          html +=
            '<button class="ui-colorpicker-ok">' +
            inst._getRegional('ok') +
            '</button>'
          html += '</div>'
        }
      ...
      this.init = function () {
        ...
        $('.ui-colorpicker-ok', part)
          .button()
          .click(function () {
            inst.close()
          })

        $('.ui-colorpicker-cancel', part)
          .button()
          .click(function () {
            inst.color = inst.currentColor.copy()
      ...
      this.repaint = function () {
        if (!inst.color.set) {
          $('.ui-colorpicker-special-none', part)
            .attr('checked', true)
            .button('refresh')
        } else if (inst.color.getAlpha() == 0) {
          $('.ui-colorpicker-special-transparent', part)
            .attr('checked', true)
            .button('refresh')
        } else {
        ...
        $('.ui-colorpicker-cancel', part).button(
          inst.changed ? 'enable' : 'disable'
        )
      }
  ...
  var Color = function () {
    ...
    const _hexify = function (number) {
      const digits = '0123456789abcdef'
      const lsd = number % 16
      const msd = (number - lsd) / 16
      const hexified = digits.charAt(msd) + digits.charAt(lsd)
      return hexified
    }
    const _rgb_to_xyz = function (rgb) {
      const r =
        rgb.r > 0.04045
    ...
    const _xyz_to_rgb = function (xyz) {
      const rgb = {
        r: xyz.x * 3.2406 + xyz.y * -1.5372 + xyz.z * -0.4986,
        g: xyz.x * -0.9689 + xyz.y * 1.8758 + xyz.z * 0.0415,
        b: xyz.x * 0.0557 + xyz.y * -0.204 + xyz.z * 1.057
      }

      rgb.r =
        rgb.r > 0.0031308
          ? 1.055 * Math.pow(rgb.r, 1 / 2.4) - 0.055
          : 12.92 * rgb.r
      rgb.g =
        rgb.g > 0.0031308
          ? 1.055 * Math.pow(rgb.g, 1 / 2.4) - 0.055
          : 12.92 * rgb.g
      rgb.b =
        rgb.b > 0.0031308
          ? 1.055 * Math.pow(rgb.b, 1 / 2.4) - 0.055
          : 12.92 * rgb.b

      return rgb
    }
    const _rgb_to_hsv = function (rgb) {
    ...
    const _hsv_to_rgb = function (hsv) {
      const rgb = {
        r: 0,
        g: 0,
        b: 0
    ...
    const _rgb_to_hsl = function (rgb) {
      const minVal = Math.min(rgb.r, rgb.g, rgb.b)
      const maxVal = Math.max(rgb.r, rgb.g, rgb.b)
      const delta = maxVal - minVal
      let del_R
    ...
    const _hsl_to_rgb = function (hsl) {
      let var_1
      let var_2
      const hue_to_rgb = function (v1, v2, vH) {
        if (vH < 0) {
          vH += 1
        }
        if (vH > 1) {
    ...
    const _xyz_to_lab = function (xyz) {
      // CIE-L*ab D65 1931
      let x = xyz.x / 0.95047
      let y = xyz.y
      let z = xyz.z / 1.08883
    ...
    const _lab_to_xyz = function (lab) {
      const lab2 = {
        l: lab.l * 100,
        a: lab.a * 255 - 128,
        b: lab.b * 255 - 128
      ...
      xyz.x =
        Math.pow(xyz.x, 3) > 0.008856
          ? Math.pow(xyz.x, 3)
          : (xyz.x - 16 / 116) / 7.787
      xyz.y =
        Math.pow(xyz.y, 3) > 0.008856
          ? Math.pow(xyz.y, 3)
          : (xyz.y - 16 / 116) / 7.787
      xyz.z =
        Math.pow(xyz.z, 3) > 0.008856
          ? Math.pow(xyz.z, 3)
          : (xyz.z - 16 / 116) / 7.787
    ...
    const _rgb_to_cmy = function (rgb) {
      return {
        c: 1 - rgb.r,
        m: 1 - rgb.g,
        y: 1 - rgb.b
    ...
    const _cmy_to_rgb = function (cmy) {
      return {
        r: 1 - cmy.c,
        g: 1 - cmy.m,
        b: 1 - cmy.y
    ...
    const _cmy_to_cmyk = function (cmy) {
      let K = 1
    ...
    const _cmyk_to_cmy = function (cmyk) {
      return {
    ...
    this.getRGB = function () {
      if (!spaces.rgb) {
        spaces.rgb = spaces.lab
          ? _xyz_to_rgb(_lab_to_xyz(spaces.lab))
          : spaces.hsv
            ? _hsv_to_rgb(spaces.hsv)
            : spaces.hsl
              ? _hsl_to_rgb(spaces.hsl)
              : spaces.cmyk
                ? _cmy_to_rgb(_cmyk_to_cmy(spaces.cmyk))
                : { r: 0, g: 0, b: 0 }
    ...
    this.getHSV = function () {
      if (!spaces.hsv) {
        spaces.hsv = spaces.lab
          ? _rgb_to_hsv(this.getRGB())
          : spaces.rgb
            ? _rgb_to_hsv(spaces.rgb)
            : spaces.hsl
              ? _rgb_to_hsv(this.getRGB())
              : spaces.cmyk
                ? _rgb_to_hsv(this.getRGB())
    ...
    this.getHSL = function () {
      if (!spaces.hsl) {
        spaces.hsl = spaces.rgb
          ? _rgb_to_hsl(spaces.rgb)
          : spaces.hsv
            ? _rgb_to_hsl(this.getRGB())
            : spaces.cmyk
              ? _rgb_to_hsl(this.getRGB())
              : spaces.hsv
                ? _rgb_to_hsl(this.getRGB())
    ...
    this.getCMYK = function () {
      if (!spaces.cmyk) {
        spaces.cmyk = spaces.rgb
          ? _cmy_to_cmyk(_rgb_to_cmy(spaces.rgb))
          : spaces.hsv
            ? _cmy_to_cmyk(_rgb_to_cmy(this.getRGB()))
            : spaces.hsl
              ? _cmy_to_cmyk(_rgb_to_cmy(this.getRGB()))
              : spaces.lab
                ? _cmy_to_cmyk(_rgb_to_cmy(this.getRGB()))
    ...
    this.getLAB = function () {
      if (!spaces.lab) {
        spaces.lab = spaces.rgb
          ? _xyz_to_lab(_rgb_to_xyz(spaces.rgb))
          : spaces.hsv
            ? _xyz_to_lab(_rgb_to_xyz(this.getRGB()))
            : spaces.hsl
              ? _xyz_to_lab(_rgb_to_xyz(this.getRGB()))
              : spaces.cmyk
                ? _xyz_to_lab(_rgb_to_xyz(this.getRGB()))
    ...
    this.distance = function (color) {
      const space = 'lab'
      const getter = 'get' + space.toUpperCase()
      const a = this[getter]()
      const b = color[getter]()
      let distance = 0
      let channel
  ...
  $.widget('vanderlee.colorpicker', {
    ...
    _create: function () {
      ...
      if (
        this.element[0].nodeName.toLowerCase() === 'input' ||
        !that.options.inline
      ) {
        ...
        $(document).mousedown(function (event) {
          ...
          if (
            that.dialog.is(event.target) ||
            that.dialog.has(event.target).length > 0
          ) {
        ...
        if (
          that.options.showOn === 'button' ||
          that.options.showOn === 'both'
        ) {
          ...
          if (that.options.buttonImageOnly && that.image) {
            ...
            that.button = $('<button type="button"></button>')
              .html(that.image || that.options.buttonText)
              .button()
        ...
        that.element
          ...
          .keyup(function (event) {
            const color = _parseColor(that.element.val())
    ...
    _setImageBackground: function () {
      if (this.image && this.options.buttonColorize) {
        this.image.css(
          'background-color',
          this.color.set ? _formatColor('RGBA', this.color) : ''
        )
      }
    },
    ...
    _setAltField: function () {
      if (
        this.options.altOnChange &&
        this.options.altField &&
        this.options.altProperties
      ) {
        let index
        let property
        const properties = this.options.altProperties.split(',')
        ...
        for (index = 0; index < properties.length; ++index) {
          ...
          switch (property) {
            ...
            case 'outline-color':
            case 'border-color':
              $(this.options.altField).css(
                property,
                this.color.set ? this.color.toCSS() : ''
              )
              break
          }
        }
        ...
        if (this.options.altAlpha) {
          $(this.options.altField).css(
            'opacity',
            this.color.set ? this.color.getAlpha() : ''
          )
        }
      }
    },
      ...
      if (!that.generated) {
        ...
        $(
          _layoutTable(layout_parts, function (cell, x, y) {
            const classes = ['ui-colorpicker-' + cell.part + '-container']
    ...
    _effectGeneric: function (element, show, slide, fade, callback) {
      ...
      if ($.effects && $.effects[that.options.showAnim]) {
        element[show](
          that.options.showAnim,
          that.options.showOptions,
          that.options.duration,
          callback
        )
      } else {
        element[
          that.options.showAnim === 'slideDown'
            ? slide
            : that.options.showAnim === 'fadeIn'
              ? fade
              : show
        ](that.options.showAnim ? that.options.duration : null, callback)
    ...
    open: function () {
      ...
      if (!that.opened) {
        ...
        that.overlay = that.options.modal
          ? new $.ui.dialog.overlay(that)
          : null
