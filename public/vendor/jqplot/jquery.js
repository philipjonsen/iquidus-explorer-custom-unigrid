...
(function (window, undefined) {
  ...
  // Support: Firefox 18+
  // "use strict";
  let // The deferred used on DOM ready
    readyList
  // A central reference to the root jQuery(document)
  let rootjQuery
  // Support: IE<9
  // For `typeof node.method` instead of `node.method !== undefined`
  const core_strundefined = typeof undefined
  ...
  const core_deletedIds = []
  const core_version = '1.9.1'
  // Save a reference to some core methods
  const core_concat = core_deletedIds.concat
  const core_push = core_deletedIds.push
  const core_slice = core_deletedIds.slice
  const core_indexOf = core_deletedIds.indexOf
  const core_toString = class2type.toString
  const core_hasOwn = class2type.hasOwnProperty
  const core_trim = core_version.trim
  ...
  jQuery.extend({
    ...
    when: function (subordinate /* , ..., subordinateN */) {
      let i = 0
      const resolveValues = core_slice.call(arguments)
  ...
  function internalData (elem, name, data, pvt /* Internal Use Only */) {
    ...
    const getByName = typeof name === 'string'
  ...
  const getSetAttribute = jQuery.support.getSetAttribute
  const getSetInput = jQuery.support.input
    ...
    toggleClass: function (value, stateVal) {
      ...
      return this.each(function () {
        if (type === 'string') {
          ...
          let state = stateVal
          const classNames = value.match(core_rnotwhite) || []
  ...
  jQuery.extend({
    ...
    removeAttr: function (elem, value) {
      ...
      let propName
      let i = 0
      const attrNames = value && value.match(core_rnotwhite)
  ...
  boolHook = {
    get: function (elem, name) {
      const // Use .prop to determine if this attribute is understood as boolean
        prop = jQuery.prop(elem, name)
      // Fetch it accordingly
      const attr = typeof prop === 'boolean' && elem.getAttribute(name)
      const detail =
        typeof prop === 'boolean'
  ...
  jQuery.event = {
    ...
    trigger: function (event, data, elem, onlyHandlers) {
      ...
      let type = core_hasOwn.call(event, 'type') ? event.type : event
      let namespaces = core_hasOwn.call(event, 'namespace')
    ...
    dispatch: function (event) {
      ...
      let j
      let handlerQueue = []
      const args = core_slice.call(arguments)
    ...
    mouseHooks: {
      ...
      filter: function (event, original) {
        ...
        const fromElement = original.fromElement
  ...
  (function (window, undefined) {
    ...
    const rsibling = /[\x20\t\r\n\f]*[+~]/
    const rnative = /^[^{]+\{\s*\[native code/
    // Easily-parseable/retrievable ID or TAG or CLASS selectors
    const rquickExpr = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/
    const rinputs = /^(?:input|select|textarea|button)$/i
    const rheader = /^h\d$/i
    const rescape = /'|\\/g
    const rattributeQuotes = /\=[\x20\t\r\n\f]*([^'"\]]*)[\x20\t\r\n\f]*\]/g
    // CSS escapes http://www.w3.org/TR/CSS21/syndata.html#escaped-characters
    const runescape = /\\([\da-fA-F]{1,6}[\x20\t\r\n\f]?|.)/g
    const funescape = function (_, escaped) {
    ...
    function matcherFromGroupMatchers (elementMatchers, setMatchers) {
      ...
      const superMatcher = function (
        ...
        const outermost = expandContext != null
  ...
  function winnow (elements, qualifier, keep) {
    ...
    if (jQuery.isFunction(qualifier)) {
      ...
      const filtered = jQuery.grep(elements, function (elem) {
  ...
  const manipulation_rcheckableType = /^(?:checkbox|radio)$/i
  ...
  jQuery.fn.extend({
    ...
    replaceWith: function (value) {
      const isFunc = jQuery.isFunction(value)
  ...
  function disableScript (elem) {
    const attr = elem.getAttributeNode('type')
  ...
  jQuery.extend({
    ...
    cleanData: function (elems, /* internal */ acceptData) {
      ...
      const deleteExpando = jQuery.support.deleteExpando
  ...
  const rnumsplit = new RegExp('^(' + core_pnum + ')(.*)$', 'i')
  const rnumnonpx = new RegExp('^(' + core_pnum + ')(?!px)[a-z%]+$', 'i')
  const rrelNum = new RegExp('^([+-])=(' + core_pnum + ')', 'i')
  const elemdisplay = { BODY: 'block' }
  const cssShow = {
    position: 'absolute',
    visibility: 'hidden',
    display: 'block'
  ...
  if (window.getComputedStyle) {
    ...
    curCSS = function (elem, name, _computed) {
      ...
      let minWidth
      let maxWidth
      const computed = _computed || getStyles(elem)
    ...
    curCSS = function (elem, name, _computed) {
      ...
      let rsLeft
      const computed = _computed || getStyles(elem)
      let ret = computed ? computed[name] : undefined
      const style = elem.style
  ...
  function actualDisplay (name, doc) {
    ...
    const display = jQuery.css(elem[0], 'display')
    elem.remove()
    return display
  }
  ...
  if (!jQuery.support.opacity) {
    jQuery.cssHooks.opacity = {
      ...
      set: function (elem, value) {
        const style = elem.style
        const currentStyle = elem.currentStyle
        const opacity = jQuery.isNumeric(value)
  ...
  function ajaxHandleResponses (s, jqXHR, responses) {
    ...
    const responseFields = s.responseFields
  ...
  const rfxnum = new RegExp('^(?:([+-])=|)(' + core_pnum + ')([a-z%]*)$', 'i')
  ...
  function propFilter (props, specialEasing) {
    let value, name, index, easing, hooks
  ...
  jQuery.fn.extend({
    ...
    offsetParent: function () {
      return this.map(function () {
        let offsetParent = this.offsetParent || document.documentElement
        while (
          offsetParent &&
